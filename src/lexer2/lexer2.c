...
static enum TokenType lexer2_classifyWord(char* s) {
    ...
    for (;;) {
        ...
        switch (t->action) {
            ...
            case A_CommentErr:
                return Lexer_StringError;
            default:
                fprintf(stderr, "lexer2_next got unhandled action %d\n", t->action);
                assert(false);
                exit(1);
        }
        state = t->nextState;
...
EMIT_TOKEN:
    ...
    *tokenType = t->tokenType;
    if (*tokenType == T_Word)
        *tokenType = lexer2_classifyWord(lexemeBuffer);
...
void lexer2_lexAll(struct Transition** syntax, const string_t sourceString, struct Vector* tokens) {
    ...
    for (;;) {
        ...
        count_t lexemeLen;
        enum Lexer_LexResult res = lexer2_next(syntax, &inputString, &tokenType, &lexemeLen, lexemeBuffer);
        fprintf(stderr, "lexer2_lexAll got token %d '%s'\n", tokenType, lexemeBuffer);
        switch (res) {
            case Lexer_Success: {
            ...
            case Lexer_UnknownError:
                fprintf(stderr, "lexer2_lexAll got an unknown error %d\n", res);
                return;
            case Lexer_StringError:
                fprintf(stderr, "lexer2_lexAll got a string error %d\n", res);
                return;
            case Lexer_RealError:
                fprintf(stderr, "lexer2_lexAll got an real-number error %d\n", res);
                return;
            case Lexer_CommentError:
                fprintf(stderr, "lexer2_lexAll got a comment error %d\n", res);
                return;
            default:
                fprintf(stderr, "lexer2_lexAll unhandled lex result %d\n", res);
                assert(false);
                return;
        }
    }
